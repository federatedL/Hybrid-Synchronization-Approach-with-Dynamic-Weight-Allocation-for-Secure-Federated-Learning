...
def calculate_client_weights(cur_round):
    ...
    for client_no in latest_client_round.keys():
        ...
        w = globals()['ratios']['client_no-1']/(delay+1)
        w = round(w, 2)
        w = round(w*100)
...
def new_global_training():
    ...
    for comm_round in range(limit):
        ...
        updated_grads, csum1 = aggregate_client_gradients(client_grads_list, comm_round)
        send_csum1_to_clients(csum1)
        print('csum1 transferred to clients')
        calculate_ds_at_clients()
        print('calculated ds at client-side')
        added_ds = combine_ds()
        print('combined client ds')
        res = aggregate_grads_ds(updated_grads, added_ds)
        global_gradient = res
