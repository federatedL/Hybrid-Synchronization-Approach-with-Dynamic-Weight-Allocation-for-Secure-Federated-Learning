    ...
    for i in range(steps):
        ...
        for client_no in latest_client_grad.keys():
            ...
            if client_grads is not None:
                ...
                w = client_weights[client_no]
                weighted_grad = client_grad * w
                lst.append(weighted_grad)

        agg_gradients.append(np.mean(lst, axis=0))
...
def threading_helper(client_no, r):
    ...
    with lock:
        ...
        updated_grads = threading_aggregate_client_gradients(client_no, client_grads, r)

        # Because the weights were multiplied by 100 for convenience
        for i, arr in enumerate(res):
            res[i] = arr/100

        send_threading_updated_gradients(client_no, updated_grads)
...
def new_global_training():
    ...
    limit = int((comms_round*percentile)/100)

    # Synchronous rounds
    for comm_round in range(limit):
        client_no = 1
        client_grads_list = []
        client_local_acc = []
        ...
        updated_grads = aggregate_client_gradients(client_grads_list, comm_round)
        print('Client gradients aggregated...')
        global_gradient = updated_grads
    ...
    # Asynchronous rounds
    print('Synchronous training ended...')
